...
public class ReadGeotiff {
    ...
    public static double getValue(double lon, double lat) throws Exception {
        try {
            ...
            return data[0] > 0 ? Math.round(data[0] * 0.54)  : 0;
        }catch (Exception e)
        {
    ...
    public static void updateGeotiff() throws IOException, FactoryException {
        ...
        // get geotiff stream
        // todo: go to env
        URL url = new URL("https://api.naonair.org/geoserver/aireel/wms?service=WMS&version=1.1.0&request=GetMap&layers=aireel%3Aaireel_indic_7m_NO2_deg&bbox=-1.9272281569407010,47.0913666061233300,-1.3443056421867574,47.3612095285679118&srs=EPSG%3A4326&styles&format=image%2Fgeotiff8&width=6144&height=2640");InputStream in = url.openStream();
